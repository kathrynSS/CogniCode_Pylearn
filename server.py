...
def download_file(file_id):
    ...
    try:
        ...
        if storage_type == 'drive' and drive_service and target_file.get('drive_file_id'):
            # 从Google Drive下载
            try:
                file_data = drive_service.download_file(target_file['drive_file_id'])
                
                # 创建响应
                response = make_response(file_data)
                response.headers['Content-Type'] = target_file.get('mime_type', 'application/octet-stream')
...
def delete_file(file_id):
    ...
    try:
        ...
        if storage_type == 'drive' and drive_service and target_file.get('drive_file_id'):
            # 从Google Drive删除
            try:
                file_deleted = drive_service.delete_file(target_file['drive_file_id'])
                if not file_deleted:
                    print(f"Failed to delete file from Google Drive: {target_file['drive_file_id']}")
            except Exception as e:
                print(f"Google Drive delete error: {str(e)}")
                # 即使Google Drive删除失败，也继续删除数据库记录
        ...
        # 从数据库删除文件记录
        success = db_manager.delete_user_file(user_id, file_id)
        ...
        if success:
            ...
            return jsonify({'success': False, 'error': 'Failed to delete file record from database'}), 500
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🚀 Starting PyLearn Development Server")
    print("="*60)
    print("📍 Main App: http://127.0.0.1:5000")
    print("🤖 Learning Chatbot: http://127.0.0.1:5000/learning_chatbot")
    print("="*60)
